	...
	public static Packet parseMessage(XmlPullParser parser) throws Exception {
		...
		while (!done) {
			...
			if (eventType == XmlPullParser.START_TAG) {
				...
				} else if (elementName.equals("consigna")) {
					consigna = new Consigna();
				} else if (elementName.equalsIgnoreCase("name")) {
					String name = parseContent(parser);
					if (name != null) {
						consigna.setName(name);
					}
				} else if (elementName.equalsIgnoreCase("size")) {
					String size = parseContent(parser);
					if (size != null) {
						consigna.setSize(size);
					}
				} else if (elementName.equalsIgnoreCase("urlConsigna")) {
					String urlConsigna = parseContent(parser);
					if (urlConsigna != null) {
						consigna.setUrlConsigna(urlConsigna);
					}
				} else if (elementName.equalsIgnoreCase("urlDownload")) {
					String urlDownload = parseContent(parser);
					if (urlDownload != null) {
						consigna.setUrlDownload(urlDownload);
					}
