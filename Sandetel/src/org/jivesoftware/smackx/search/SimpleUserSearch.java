	...
	protected void parseItems(XmlPullParser parser) throws Exception {
		...
		while (!done) {
			if (parser.getAttributeCount() > 0) {
				...
				valueList.add(jid);
				ReportedData.Field field = new ReportedData.Field("jid",
						valueList);
				fields.add(field);
			}
			...
			if (eventType == XmlPullParser.START_TAG
				...
				List<String> valueList = new ArrayList<String>();
				valueList.add(value);
				ReportedData.Field field = new ReportedData.Field(name,
						valueList);
				fields.add(field);
				...
				while (cols.hasNext()) {
					ReportedData.Column column = (ReportedData.Column) cols
							.next();
				...
				if (!exists) {
					ReportedData.Column column = new ReportedData.Column(name,
							name, "text-single");
