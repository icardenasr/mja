...
public class NotificationManager implements OnInitializedListener,
	...
	public <T extends NotificationItem> void updateNotifications(
		...
		if (!iterator.hasNext()) {
			...
			notification.setSmallIcon(provider.getIcon());
			notification.setTicker(ticker);
			notification.setWhen(System.currentTimeMillis());
			notification.setDeleteIntent(clearNotifications);
	...
	private void setNotificationDefaults(
			NotificationCompat.Builder notification, boolean vibro, Uri sound,
			int streamType) {
	...
	private void updateMessageNotification(MessageItem ticker) {
		...
		if (messageNotifications.isEmpty()) {
			...
			if (MUCManager.getInstance().hasRoom(message.getAccount(),
				...
				chatViews
						.setImageViewResource(R.id.icon, R.drawable.avatar_4_8);

				// Bitmap bitmap = AvatarManager.getInstance().getUserBitmap(
				// message.getUser());
			...
			Collection<MessageItem> messages = MessageManager.getInstance()
					.getMessages(message.getAccount(), message.getUser());
			MessageItem messageItemNotify = null;
			for (MessageItem messageItem : messages) {
				if (messageItem.getText().equals(message.getText())) {
			...
			if (ChatManager.getInstance().isShowText(message.getAccount(),
				...
				if (messageItemNotify != null
					...
					text = application.getResources().getString(
			...
			// R.drawable.avatar_4_8);

			// Bitmap bitmap =
			// BitmapFactory.decodeResource(application.getResources(),R.drawable.avatar_4_8);
			Bitmap bitmap = AvatarManager.getInstance().getUserBitmap(
					message.getUser());
			...
			Bitmap b = BitmapFactory.decodeByteArray(byteArray, 0,
					byteArray.length);
			Bitmap newBitmap = Bitmap.createScaledBitmap(b, 48, 48, false);

			// Bitmap newBitmap = Bitmap.createBitmap(bitmap);

			// Bitmap battery = bitmap.copy(Bitmap.Config.ARGB_8888, true); //
			// workaround
			// chatViews.setImageViewBitmap(R.id.icon, newBitmap);

			// chatViews.setImageViewResource(R.id.icon, R.drawable.avatar_4_8);

			Builder notification = new Builder(
					application.getApplicationContext());
			if (Application.SDK_INT >= 14 && SettingsManager.eventsPersistent()) {
				// Ongoing icons are in the left side, so hide this one.
				notification
						.setSmallIcon(connected > 0 ? R.drawable.ic_stat_message
								: R.drawable.ic_stat_message_offline);
				notification.setLargeIcon(newBitmap);
				notification.setWhen(Long.MIN_VALUE);
			} else {
			...
			// notification.setContent(chatViews);
			...
			if (Application.SDK_INT >= 11) {
				not = notification.build();
				not.contentView = chatViews;
				not.tickerText = message.getUser().split("@")[0] + ": " + text;
			} else {
				notification.setContentTitle(RosterManager.getInstance()
						.getName(message.getAccount(), message.getUser()));
				notification.setContentText(text);
			...
			try {
				notify(CHAT_NOTIFICATION_ID, not, message.getAccount(),
						message.getUser());
		...
		if (Application.SDK_INT >= 14 && SettingsManager.eventsPersistent()) {
			...
			if (messageNotifications.isEmpty()) {
				// persistentNotification.icon = connected > 0 ?
				// R.drawable.ic_stat_normal
			...
			if (messageNotifications.isEmpty()) {
				...
				// persistentNotification.when = Application.SDK_INT >= 9 ?
				// -Long.MAX_VALUE
				// : Long.MAX_VALUE;
			}
		}
	...
	private void notify(int id, Notification notification, String account,
			String user) {
		...
		if ((account == null && user == null)
				|| (account != null && user != null && ChatManager
						.getInstance().getSound(account, user) != null)) {
	 ...
	 * @param ticker
	 */
	private void updateNotification(Builder notification, MessageItem ticker) {
		if (ticker == null)
			return;
		...
		if (ChatManager.getInstance().isShowText(ticker.getChat().getAccount(),
				ticker.getChat().getUser()))
			notification.setTicker(trimText(ticker.getText()));
	}
