...
public class PresenceManager implements OnArchiveModificationsReceivedListener,
	 ...
	 * @throws NetworkException
	 */
	public void destroyRoom(String account, String room, Boolean destroy)
			throws NetworkException {
		MUCManager.getInstance().removeRoom(account, room);
		MessageManager.getInstance().closeChat(account, room);
		NotificationManager.getInstance().removeMessageNotification(account,
				room);

		if (destroy) {
			MUCManager.getInstance().destroyRoom(account, room);
		}

		Activity rootActivity = ActivityManager.getInstance().getLastActivity();
		...
		if (rootActivity != null) {
			...
			builder.setMessage(
					String.format(
							rootActivity.getString(R.string.destroy_body),
							room.split("@")[0])).setTitle(
					rootActivity.getString(R.string.destroy_title));
	...
	public void onPacket(ConnectionItem connection, String bareAddress,
		...
		Boolean isDestroyRoom = false;
		if (packet != null && packet.getExtensions() != null) {
			for (PacketExtension pe : packet.getExtensions()) {

				if (pe instanceof MUCUser) {

					if (((MUCUser) pe).getDestroy() != null) {
						isDestroyRoom = true;
						break;
					}
