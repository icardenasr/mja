...
public class MessageManager implements OnLoadListener, OnPacketListener,
	...
	public void sendMessage(String account, String user, String text,
		...
		if (consigna == null) {
			messageItem = chat.newMessage(text);
		} else {
			messageItem = chat.newMessage(text, consigna);
		}
	 ...
	 * @param text
	 */
	public MessageItem createMessageLocaly(String account, String user,
			String text, Consigna consigna) {
		AbstractChat chat = getChat(account, user);
		if (chat == null)
			chat = createChat(account, user);
		MessageItem messageItem = chat.newMessage(text);
		if (consigna != null) {
			consigna.setAvailableToSend(false);
			messageItem.setConsigna(consigna);
		}
	...
	// * @param text
	// */
	// public void sendMessageFile(String account, String user, String text,
	// String pathFile) {
	//
	// // String text = "Enviado correctamente";
	// AbstractChat chat = getChat(account, user);
	// if (chat == null)
	// chat = createChat(account, user);
	// MessageItem messageItem = chat.newMessage(text);
	// if (pathFile != null) {
	// messageItem.setTag(pathFile);
	// }
	//
	// messageItem.setConsigna(true);
	//
	// chat.sendQueueFile(messageItem);
	// }
	...
	public void onPacket(ConnectionItem connection, String bareAddress,
		...
		for (AbstractChat chat : chats.getNested(account).values())
			if (chat.onPacket(connection, bareAddress, packet)) {
				processed = true;
				break;
			}
		...
		if (!processed && packet instanceof Message) {
			...
			// Comprobamos si el usuario o el grupo existe en nuestra base
			// de datos
			if (checkIfExist(account, user)) {
				...
				for (PacketExtension packetExtension : message.getExtensions())
					if (packetExtension instanceof MUCUser)
						return;
				createChat(account, user).onPacket(connection, bareAddress,
						packet);
			}
		}
		...
		try {
			...
			if (abstractChat != null) {
				...
				for (MessageItem messageItem : abstractChat.getMessages()) {
					...
					out.write("<b>");
					out.write(StringUtils.escapeHtml(name.split("@")[0]));
					out.write("</b>&nbsp;(");
	 ...
	 * @return Collection with ids for specified messages.
	 */
	static Collection<Long> getConsignaIds(Collection<Consigna> consignaItems,
			boolean clearId) {
		ArrayList<Long> ids = new ArrayList<Long>();
		for (Consigna consigna : consignaItems) {
	...
	private Boolean checkIfExist(String account, String userRoom) {

		Boolean result = false;
		String name = VCardManager.getInstance()
				.getName(userRoom.split("/")[0]);

		if (name != null && !"".equalsIgnoreCase(name)) {
			result = true;
		} else {

			result = MUCManager.getInstance().hasRoom(account, userRoom);
		}
