...
public class MUCManager implements OnLoadListener, OnPacketListener {
	...
	public void createRoom(String account, String room, String nickname,
		...
		if (join)
			joinRoom(account, room, true, false);
	}

	/**
	 * Destroy existed room.
	 * 
	 * @param account
	 * @param room
	 * @param nickname
	 * @param password
	 */
	public void destroyRoom(String account, String room) {
		...
		iq.setTo(room);
		try {
			ConnectionManager.getInstance().sendPacket(account, iq);
	 ...
	 * @param password
	 */
	public void getInfoRoom(String account, String room) {
		...
		Application.getInstance().addCheckRooms(iq.getPacketID());
		try {
			ConnectionManager.getInstance().sendPacket(account, iq);
		} catch (NetworkException e) {
			// TODO Auto-generated catch block
	...
	public void joinRoom(final String account, final String room,
			boolean requested) {
		joinRoom(account, room, requested, true);
	}
	 ...
	 */
	public void joinRoom(final String account, final String room,
			boolean requested, boolean checkIfExist) {
		...
		if (checkIfExist) {
			...
			MUCManager.getInstance().getInfoRoom(account, room);
	...
	public void onPacket(ConnectionItem connection, String bareAddress,
		...
		if (packet instanceof DiscoverInfo) {
			...
			DiscoverInfo dicoDiscoverInfo = (DiscoverInfo) packet;
			if (dicoDiscoverInfo != null) {

				if (Application.getInstance().waitingForRoom(
						dicoDiscoverInfo.getPacketID())) {

					String to = null, from = null;
					from = dicoDiscoverInfo.getFrom();
					to = dicoDiscoverInfo.getTo();

					XMPPError xmppError = dicoDiscoverInfo.getError();
					if (xmppError != null && xmppError.getCode() == 404
							&& from != null && to != null) {
						// MUCManager.getInstance().removeRoom(to, from);
						// MessageManager.getInstance().closeChat(to, from);
						// NotificationManager.getInstance()
						// .removeMessageNotification(to, from);

						try {
							PresenceManager.getInstance().destroyRoom(to, from,
									false);
						} catch (NetworkException e) {
							// TODO Auto-generated catch block
							e.printStackTrace();
			...
			if (roomChat == null || !roomChat.getState().inUse()) {
				...
				if (inviter == null)
					inviter = bareAddress;
				inviteProvider
						.add(new RoomInvite(account, bareAddress, inviter,
								mucUser.getInvite().getReason(), mucUser
										.getPassword()), true);
			}
		}
	}
