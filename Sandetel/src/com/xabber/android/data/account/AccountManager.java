...
public class AccountManager implements OnLoadListener, OnWipeListener {
	 ...
	 * List of accounts.
	 */
	private Map<String, AccountItem> accountItems;

	/**
	 * List of enabled account.
	 */
	private Collection<String> enabledAccounts;
	...
	private IActionAfterLoad iActionAfterLoad;
	...
	private void onLoaded(Collection<SavedStatus> savedStatuses,
		...
		if (iActionAfterLoad != null) {

			iActionAfterLoad.actionAlreadyLoad();
			iActionAfterLoad = null;
		}
	}
	 ...
	 * @param account
	 */
	private void removeAccountWithoutCallback(String account) {
		...
		final String accountAux = account;
		Application.getInstance().runInBackground(new Runnable() {
			@Override
			public void run() {
				AccountTable.getInstance().remove(accountAux,
						accountItem.getId());
			}
		});
	...
	public void removeAllAccount() {
		...
		for (Map.Entry<String, AccountItem> entry : accountItems.entrySet()) {
			AccountItem account = entry.getValue();
	...
	public void setEnable(String account, boolean enable) {
		setEnable(account, enable, true);
	}

	public void setEnable(String account, boolean enable, boolean syncable) {
		AccountItem accountItem = getAccount(account);
		updateAccount(account, accountItem.getConnectionSettings().isCustom(),
				accountItem.getConnectionSettings().getHost(), accountItem
						.getConnectionSettings().getPort(), accountItem
				...
				accountItem.isStorePassword(), accountItem
						.getConnectionSettings().getPassword(), accountItem
						.getConnectionSettings().getResource(),
				accountItem.getPriority(), enable, accountItem
						.getConnectionSettings().isSaslEnabled(), accountItem
						.getConnectionSettings().getTlsMode(), accountItem
						.getConnectionSettings().useCompression(), accountItem
						.getConnectionSettings().getProxyType(), accountItem
	...
	public void setiActionAfterLoad(IActionAfterLoad iActionAfterLoad) {
		this.iActionAfterLoad = iActionAfterLoad;
	}

	public IActionAfterLoad getiActionAfterLoad() {
		return iActionAfterLoad;
	}
	...
	public void setAvailableStatusAllAcount() {

		AccountTable.getInstance().updateStatus();
