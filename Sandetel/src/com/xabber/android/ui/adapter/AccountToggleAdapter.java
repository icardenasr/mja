...
public class AccountToggleAdapter implements UpdatableAdapter {
	...
	public AccountToggleAdapter(ManagedListActivity activity,
	...
	public void rebuild() {
		...
		Collection<String> listAccount = AccountManager.getInstance()
				.getAccounts();

		// UsuarioDTO accountLoged = (UsuarioDTO)
		// UtilPreferences.getPreferences(
		// activity, UsuarioWS.KEY_USUARIO, UsuarioDTO.class);
		// int lastPosition = listAccount.size() - 1;
		//
		// int i = 0;

		// if (accountLoged != null && accountLoged.getNick() != null) {
		for (String account : listAccount) {
			// //
			// // if (i == lastPosition) {
			// if (account.contains(accountLoged.getNick())) {
		...
		// // }
		// // }
		AccountManager.getInstance().onAccountsChanged(accounts);
	...
	public void onChange() {
		...
		for (int index = 0; index < accounts.size(); index++) {
			...
			if (accountItem != null) {
				...
				if (activity != null && activity instanceof ContactList) {

					String stateOwn = activity.getString(accountItem.getState()
							.getStringId());
					((ContactList) activity).updateStatusBar(stateOwn);

				}

				StatusMode statusMode = accountItem.getDisplayStatusMode();
				int colorLevel = AccountManager.getInstance().getColorLevel(
						account);
				...
				accountViewHolder.statusMode.getBackground().setLevel(
						colorLevel);
				accountViewHolder.statusMode
						.setImageLevel(statusMode.ordinal());
